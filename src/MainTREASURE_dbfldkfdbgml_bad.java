...
public class MainTREASURE_dbfldkfdbgml_bad {
	...
	private static ArrayList<double[]> intersection(Rectangle rectangle, ArrayList<int[]> island) {
		...
		Stack<Integer> stackOfFromIndex = new Stack<>();
			...
			if (!isInside(p1, rectangle) && !isInside(p2, rectangle)) {
				...
				for (int j = 0; j < rectangle.getAllPoints().size(); j++) {
					...
					if (isIntersect(p1, p2, p3, p4)) {
						...
						find = true;
						inIndexOfRectangle = max(j, inIndexOfRectangle);
					}
				}
				if(!stackOfFromIndex.isEmpty() & find) {
					ArrayList<double[]> rectanglePointOfIntersection = getRectanglePointOfIntersection(rectangle, stackOfFromIndex.peek(), inIndexOfRectangle);
					ret.addAll(rectanglePointOfIntersection);
				}
	...
	private static ArrayList<double[]> getRectanglePointOfIntersection(Rectangle rect, int from, int to) {
		ArrayList<double[]> ret = new ArrayList<>();
		
		if(from < to) {
			for(int i = from + 1 ; i <= to ; i++) {
				int[] temp = rect.getAllPoints().get(i);
				ret.add(new double[] { temp[0], temp[1] });
			}
		}else {
			for(int i = from + 1 ; i <= to + 4 ; i++) {
				int[] temp = rect.getAllPoints().get(i % 4);
				ret.add(new double[] { temp[0], temp[1] });
			}
		}
		return ret;
	...
	private static double[] getIntersectOf(int[] p1, int[] p2, MainTREASURE_dbfldkfdbgml_bad.Rectangle rectangle) {
		for (int j = 0; j < rectangle.getAllPoints().size(); j++) {
			int[] p3 = rectangle.getAllPoints().get(j);
			int[] p4 = rectangle.getAllPoints().get((j + 1) % rectangle.getAllPoints().size());
			if (isIntersect(p1, p2, p3, p4)) {
